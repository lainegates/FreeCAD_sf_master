...
#        if (len(self.node) > 1):   
#            self.commit("Create Wire",
#                        partial(Base.makeWire, self.node, closed,
#                                face=False, support=self.support , fname = self.shapeType))
        tmpObj = None
...
#            tmpObj = Base.addLines2Document(self.node , True , face=False, support=self.support , fname = self.shapeType)
        ...
        if self.shapeType!= 'MaterialLine':
            tmpObj = Base.addPolyLine2Document(self.shapeType, ifStore2Database = True , pointsList=self.node , closed=True)
