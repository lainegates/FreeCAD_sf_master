...
#        if not (self.boundaryLines and len(self.boundaryLines)>0) :
    ...
    def write2DLFile(self):
        ...
        wholePath = __workbenchPath__ + '\\Ff.c'
...
class DCChangesConfirm(BaseStore):
    '''
...
#        print type
        if type == 'MaterialLine':
            self.materialLines.append(obj)
        if type == 'FixedPoint':
            self.fixedPoints.append(obj)
        elif type == 'LoadingPoint':
            self.loadingPoints.append(obj)
        elif type == 'MeasuredPoint':
            self.measuredPoints.append(obj)
        elif type == 'HolePoint':
            self.holePoints.append(obj)
    ...
    def organizeContent(self):
        ...
        self.newFile.append('%d\n%d\n%d\n%d\n%d\n%d'%(len(self.materialLines) , 0 \
                                , len(self.fixedPoints) , len(self.loadingPoints) \
                                , len(self.measuredPoints) , len(self.holePoints)))
        ...
        for p in self.fixedPoints:
            center = p.ViewObject.Center
            self.newFile.append('%f %f %f %f'%(center[0],center[1] , center[0],center[1]))        
        
        self.__addPoints(self.loadingPoints)
        self.__addPoints(self.measuredPoints)
        self.__addPoints(self.holePoints)
        
        if len(self.holePoints)>0:
            self.newFile.append('0')
        else:
            self.newFile.append('0\n0 0')
    ...
    def __addPoints(self , points):
        ...
        for p in points:
            center = p.ViewObject.Center
            self.newFile.append('%f %f'%(center[0],center[1]))
...
FreeCADGui.addCommand('DDA_DCChangesConfirm', DCChangesConfirm())
