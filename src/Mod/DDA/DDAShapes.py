...
#        if obj :
#            FreeCAD.Console.PrintError('shape just drawn found\n')
        
#        print 'iswire : ' , self.isWire , ' length of nodes : ' , len(self.node) , ' continue : ' , cont
        
        if flag:  # 绘制结束时再存入database
            assert obj            
            obj.Material = self.getMaterialNumber()
            self.clearDialog()
...
#        self.dataTable.saveDataSignal.connect(self.save2Database)
    ...
    def save2Database(self):
        ...
        for i in range( self.table.rowCount() ):
            ...
            tuple = (self.table.cellWidget(i,0).currentIndex(),array[1],array[2],array[3],array[4],array[5],array[6])
